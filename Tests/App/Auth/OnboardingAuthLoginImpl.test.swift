...
class OnboardingAuthLoginImplTests: XCTestCase {
    ...
    func testInvalidURL() throws {
        let result = login.open(authDetails: authDetails, sender: sender)
        let viewController = try XCTUnwrap(FakeOnboardingAuthLoginViewController.lastCreated)
        try viewController.resolver.fulfill(XCTUnwrap(URL(string: "apporohome://auth-callback?no_code_here=true")))
    ...
    func testSuccess() throws {
        let result = login.open(authDetails: authDetails, sender: sender)
        let viewController = try XCTUnwrap(FakeOnboardingAuthLoginViewController.lastCreated)
        try viewController.resolver.fulfill(XCTUnwrap(URL(string: "apporohome://auth-callback?code=code_123")))
