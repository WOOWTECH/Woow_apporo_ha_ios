...
<p align="center">
  <strong>Apporo aiot 生態系的白牌 Home Assistant 隨行 App</strong><br/>
...
| | |
|---|---|
| **Bundle ID** | `com.apporo.aiot`(Release)/ `com.apporo.aiot.dev`(Debug)——與 Android 對齊 |
| **URL scheme** | `apporoaiot://`(深連結 + OAuth callback);Debug 版註冊的是 `apporoaiot-dev://` |
| **OAuth client** | `https://aiot.apporo.ai/ios`(`AppConstants.OAuth.clientID`;真相來源 [`docs/ios/index.html`](docs/ios/index.html),同時宣告 `apporoaiot://auth-callback` 與 `apporoaiot-dev://auth-callback`)——**尚未上線**:`aiot.apporo.ai` 目前沒有 DNS 紀錄,在發佈前登入一定失敗 |
...
flowchart LR
    subgraph iPhone["Apporo aiot app(iOS)"]
        WV["WKWebView<br/>HA 前端"] <--> BUS["JS ↔ Swift<br/>message bus"] <--> N["原生外殼<br/>onboarding · OAuth · 感測器 ·<br/>apporoaiot:// 深連結 · widgets"]
    end
    WV -- "HTTPS / WebSocket" --> HA["Home Assistant 伺服器<br/>(客戶自架)"]
    N -.->|"IndieAuth client 頁<br/>https://aiot.apporo.ai/ios"| PAGE["宣告<br/>apporoaiot://auth-callback<br/>apporoaiot-dev://auth-callback"]
```

兩個平台在同一個品牌主機下各有一頁 client_id(`/ios` 與 `/android`)——HA 伺服器會實際
抓取該頁,並以頁內 `<link rel="redirect_uri">` 驗證 OAuth redirect(IndieAuth)。
...
| 實伺服器 OAuth 全鏈路(完整原生 onboarding、儀表板、深連結) | ✅ 2026-08-16([報告](docs/verification/phase4-report.md))——跑的是**換裝前**身分(`com.apporo.home` / `apporohome://`),遷移後未重跑 |
| 以 `com.apporo.aiot` / `apporoaiot://` 重跑 OAuth 驗收(前提:`https://aiot.apporo.ai/ios` 已上線) | ⏳ 待辦 |
| 實機 + 8 大類冒煙 | ⏳ 待辦 |
